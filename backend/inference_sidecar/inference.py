...
def predict(image: Image.Image) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    gen_ids = gen_ids[: len(probs)]

    try:
        conf_groups, overall = _field_confidences(gen_ids, probs, tokenizer)
    except Exception:
        conf_groups, overall = {}, None
    ...
    return parsed, {"groups": conf_groups, "overall": overall}
